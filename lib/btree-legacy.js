(function() {
	...
	// Method to always return arrays (for backward compatibility)
	btree.prototype.findAll = function(key) {
		try {
			// Validate input
			if (key === undefined || key === null) {
				throw new Error('Key cannot be undefined or null');
	...
	// Helper methods to get nodes instead of values (for internal use)
	btree.prototype.getMinimumNode = function() {
		try {
			if (!this.root) {
				throw new Error('Cannot find minimum in empty tree');
			}
	...
	btree.prototype.getMaximumNode = function() {
		try {
			if (!this.root) {
				throw new Error('Cannot find maximum in empty tree');
			}
