...
class BTree {
    ...
    /**
     * Internal method to add item to tree with AVL balancing
     * @private
     */
    ...
    /**
     * Internal method to remove item from tree with AVL balancing
     * @private
     */
    _remove(node, key) {
        if (!node) {
    ...
    balance() {
        try {
            ...
            // AVL tree is automatically balanced during insertions/deletions
            // This method ensures the tree is properly balanced
            const items = this.items();
            if (!items || items.length === 0) {
                return this;
            }

            // Rebuild tree with proper AVL balancing
            this.root = null;
    ...
    /**
     * Check if tree is properly balanced (AVL property)
     * @returns {boolean} True if tree is balanced
     */
    isAVLBalanced() {
        return this._isAVLBalanced(this.root);
    }

    /**
     * Internal method to check AVL balance
     * @private
     */
    _isAVLBalanced(node) {
        if (!node) {
        ...
        return this._isAVLBalanced(node.left) && this._isAVLBalanced(node.right);
    }
     ...
     * @returns {number} Balance factor (-1, 0, or 1 for balanced tree)
     */
    getBalanceFactor() {
